...
class ReadLevelModel(nn.Module):
    def __init__(self, model_type, dropout_rate=0.5, hidden_size=128,
        ...
        # 新增适配层（可选）
        self.resnet_fusion_adapter = nn.Linear(1024, 256)  # 双路ResNet拼接后的维度
    ...
    def forward(self, features):
        ...
        # Debug: Print shapes after feature extraction
        #kmer shape:  torch.Size([1540, 5])
        #kmer shape:  torch.Size([1000, 5])
        #print("kmer shape: ", kmer.shape)#kmer shape:  torch.Size([2560, 5])
        #qual shape:  torch.Size([1540, 5])
        #qual shape:  torch.Size([1000, 5])
        #print("qual shape: ", qual.shape)#qual shape:  torch.Size([2560, 5])
        #mis shape:  torch.Size([1540, 5])
        #mis shape:  torch.Size([1000, 5])
        #print("mis shape: ", mis.shape)#mis shape:  torch.Size([2560, 5])
        #ins shape:  torch.Size([1540, 5])
        #ins shape:  torch.Size([1000, 5])
        #print("ins shape: ", ins.shape)#ins shape:  torch.Size([2560, 5])
        #dele shape:  torch.Size([1540, 5])
        #dele shape:  torch.Size([1000, 5])
        #print("dele shape: ", dele.shape)#dele shape:  torch.Size([2560, 5])
        #signals shape:  torch.Size([1540, 5, 65])
        #signals shape:  torch.Size([1000, 5, 65])
        #print("signals shape: ", signals.shape)#signals shape:  torch.Size([2560, 5, 65])
        ...
        if self.raw and self.basecall:
            ...
            # Debug: Print shape before Full_NN (raw + basecall)
            # z shape before Full_NN (raw + basecall):  torch.Size([1540, 512])
            # z shape before Full_NN (raw + basecall):  torch.Size([1000, 512])
        ...
        # Output shape after sigmoid:  torch.Size([1540, 1])
        # Output shape after sigmoid:  torch.Size([1000, 1])
        #print("Output shape after sigmoid: ", out_.shape)  # Output shape after sigmoid:  torch.Size([2560, 1])
...
class SiteLevelModel(nn.Module):
    ...
    def forward(self, features):
        ...
        # Debug: Print shape before passing to read level model
        #features shape before passing to read level model:  torch.Size([1540, 70, 5])
        #features shape before passing to read level model:  torch.Size([1000, 70, 5])
        #features shape before passing to read level model:  torch.Size([2560, 70, 5])
        #print("features shape before passing to read level model: ", features.shape)

        probs = self.read_level_model(features).view(-1, min_cov)

        # Debug: Print shape after read level model
        #probs shape after read level model:  torch.Size([77, 20])
        #probs shape after read level model:  torch.Size([50, 20])
        #print("probs shape after read level model: ", probs.shape)
