...
from MultiNano.utils.constants import max_seq_len, use_cuda, FloatTensor, code2base
from MultiNano.utils.MyDataSet_txt import read_level_features_line
...
def argparser():
    ...
    parser.add_argument("--model", default="/root/autodl-tmp/1111New/syn/syn_ni_all/model_states_1.pt", required=False, help="model file for test")
    parser.add_argument("--input_file", default="/root/autodl-tmp/syn/test/rl_unmod_rep2_11mer_65_features.label.sort.aggre.c15.c20_1000.tsv", required=False, help="input features file")
    #/root/autodl-tmp/syn/test/rl_mod_rep2_11mer_65_features.label.sort.aggre.c15.c20_1000.tsv
    #/root/autodl-tmp/syn/test/rl_unmod_rep2_11mer_65_features.label.sort.aggre.c15.c20_1000.tsv
...
def _predict(features_batch, model, device=0):
    ...
    for idx in range(len(read_probs)):
        prob = round(read_probs[idx], 6)
        meth = 1 if prob >= 0.5 else 0
...
def _write_predstr_to_file(write_fp, predstr_q):
    ...
    with open(write_fp, 'w') as wf:
        wf.write("chrom\tpos\talignstrand\tloc_in_re\treadname\tstrand\tkmer\tprob\tmethylation\n")
        while True:
            if predstr_q.empty():
                time.sleep(0.1)
                continue
