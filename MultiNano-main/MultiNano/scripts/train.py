...
from MultiNano.utils.MyDataSet_txt import MyDataSetTxt, MyDataSetTxt2, generate_offsets, clear_linecache
from MultiNano.utils.hct_sample import *
from MultiNano.utils.pytorchtools import EarlyStopping
from MultiNano.utils.constants import use_cuda
from MultiNano.utils.data_utils import count_line_num
    ...
    with torch.no_grad():
        for _ in range(n_iters):
            ...
            for batch_features_all in tqdm(val_dl):
                ...
                y_pred = y_pred.squeeze(-1)
                #loss = loss_func(y_pred.to(torch.float), y_true.to(torch.float))  # sigmoid
                y_pred = y_pred.reshape(-1)  # 确保预测值是1D张量
                y_true = y_true.reshape(-1)  # 确保标签是1D张量
                #loss = loss_func(y_pred, y_true)
                loss = loss_func(y_pred, y_true.float())  # 添加显式类型转换
...
def train(args, ):
    ...
    if "CUDA_VISIBLE_DEVICES" not in os.environ:
        os.environ["CUDA_VISIBLE_DEVICES"] = "0,1,2"  # 默认使用第一个 GPU
    if use_cuda:
    ...
    print('************* Model loader')
    #model = TransformerModel(args.model_type,  args.seq_lens, args.signal_lens, args.embedding_size, args.num_heads , args.num_layers, args.hidden_size, args.dropout_rate)
    model = SiteLevelModel(args.model_type, args.dropout_rate, args.hidden_size, args.seq_lens, args.signal_lens,
                           args.embedding_size)
    #    def __init__(self, model_type, seq_len=5, signal_len=65, embedding_size=5, num_heads=5, num_layers=4,
    ...
    for epoch in range(args.epochs):
        ...
        # early-stopping
        early_stopping(val_results_epoch['acc'], model, optimizer)
        if curr_best_epoch_acc > val_results_epoch['acc']:
...
def argparser():
    ...
    parser.add_argument("--train_file",
                        default=r"/root/autodl-tmp/syn/train/syn_rep1_sl.11mer_65_features.train_0.8_0k.txt",
                        required=False)
    parser.add_argument("--valid_file",
                        default=r"/root/autodl-tmp/syn/train/syn_rep1_sl.11mer_65_features.valid_0.2_0k.txt",
                        required=False)
    ...
    parser.add_argument("--train_option", default=0, type=int, required=False, help='input dataset option: [0] --train_file --valide_hdf5_dir  [1] --train_hdf5_dir --  [2] --train_val_pos_file --train_val_neg_file  [3] --train_file --valid_file')

    parser.add_argument("--sample", type=str, help="sample [no_sample for option[0,1], unm_undersample for option[2]]")

    parser.add_argument("--save_dir", default="/root/autodl-tmp/111111111transfer/arab/model/jun_all2", required=False, help='modelTrans.pt save dir')
